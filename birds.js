...
class BirdX {
    ...
    async checkAndJoinGuild(telegramauth) {
        const headers = { 
            ...this.headers, 
            "Telegramauth": `tma ${telegramauth}`
        };
        ...
        try {
            const guildStatusResponse = await axios.get("https://api.birds.dog/guild/me", { headers });
            
            if (guildStatusResponse.data === null) {
                try {
                    const joinResponse = await axios.get("https://api.birds.dog/guild/join/67198910d6afd34638ff1718", { headers });
                    
                    if (joinResponse.data === "ok") {
                        this.log("Gia nhập thành công GUILD + 5% POINT BONUS", 'success');
                    }
                } catch (joinError) {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                if (apiResult) {
                    ...
                    await this.checkAndJoinGuild(telegramauth);
